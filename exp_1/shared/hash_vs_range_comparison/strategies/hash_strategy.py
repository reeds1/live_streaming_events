"""
Hash Sharding Strategy Implementation
Shards data by user_id: shard_id = hash(user_id) % 4
"""
...
class HashShardingStrategy(ShardingStrategy):
    ...
    def _get_shard_id(self, user_id: int) -> int:
        """
        Core routing logic: Calculate shard ID from user_id
        
        Why use hash()?
        - Ensures even distribution
        - Deterministic (same user_id always goes to same shard)
        """
    ...
    def save_coupon_result(self, coupon_result: CouponResult) -> bool:
        ...
        Steps:
        1. Calculate shard_id from user_id
        ...
        try:
            with conn.cursor() as cursor:
                ...
                logger.debug(f"Saved to shard {shard_id}: user {coupon_result.user_id}")
                return True
    ...
    def query_user_coupons(self, user_id: int) -> List[CouponResult]:
        """
        Query user's coupons - HASH ADVANTAGE!
        
        Why fast?
        - Only need to query ONE shard (determined by user_id)
        - No cross-shard aggregation needed
        """
    ...
    def query_room_orders(self, room_id: int, limit: int = 100) -> List[CouponResult]:
        ...
        Why slow?
        - room_id doesn't determine shard
    ...
    def query_time_range_orders(self, start_time: datetime, 
        ...
        """
        Query time range - HASH DISADVANTAGE!
        
        Same issue as query_room_orders:
            ...
            try:
                with conn.cursor() as cursor:
                    ...
                    stats.append(ShardingStats(
                        ...
                        avg_response_time=0.0,  # TODO: Implement monitoring
                        cpu_usage=0.0,
                        io_usage=0.0,
                        connection_count=0
                    ))
    ...
    def get_strategy_name(self) -> str:
        return "Hash Partitioning (by user_id)"
