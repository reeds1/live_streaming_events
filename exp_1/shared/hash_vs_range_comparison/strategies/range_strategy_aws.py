"""
Range Sharding Strategy for AWS RDS
...
class RangeShardingStrategyAWS(ShardingStrategy):
    """Range-based sharding strategy on AWS RDS"""
    
    def __init__(self, num_shards: int = 4):
        self.num_shards = num_shards
        self.pool = connection_pool_aws
        self.table_name = "coupon_results_range"  # Use dedicated table for Range
        self.shard_ranges = [
            (1, 1000),
            (1001, 2000),
            (2001, 3000),
    ...
    def _get_shard_id(self, room_id: int) -> int:
        """Calculate shard ID from room_id"""
    ...
    def query_user_coupons(self, user_id: int) -> List[CouponResult]:
        """Query user's coupons - RANGE DISADVANTAGE on AWS!"""
        all_results = []
    ...
    def query_room_orders(self, room_id: int, limit: int = 100) -> List[CouponResult]:
        """Query room orders - RANGE ADVANTAGE on AWS!"""
    ...
    def query_time_range_orders(self, start_time: datetime, 
        ...
        """Query time range - must query all shards"""
        all_results = []
        for shard_id in range(self.num_shards):
            conn = self.pool.get_shard_connection(shard_id)
            try:
    ...
    def get_strategy_name(self) -> str:
        return "Range Partitioning (AWS RDS)"
